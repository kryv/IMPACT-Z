!!QZ20120210    --- changes 2012-02-10 -------------------------------------------------------------
!!  up to 7 multipoes in place:
!!  4-pole(quadrupole),6-pole(sextupole),8-pole(octupole),10-pole(decapole),12-pole(duodecapole)
!!  14-pole, and 16 pole
!!  previous version (2011-06-02) up to 3 multipoles (quad, sext,oct)
!!  original version from J.Q. up to 3 multipoles (sext,oct,decapole)
!!QZ20120210    --- changes 2012-02-10 -------------------------------------------------------------
...
! Version: 1.0
! Author: Ji Qiang, LBNL, 4/14/04
! Description: This class defines the linear transfer map and field
!              for the multipole (sextupole, octupole, decapole)
!              beam line elment.
! Comments: This element has to be used together with the Lorentz integrator
!           The map does not work here
!----------------------------------------------------------------
      module Multipoleclass
        ...
        type Multipole
          ...
          ! Param(1) : zedge
          !      (2) : field strength for quadrupole
          !      (3) : for sextupole
          !      (4) : for octupole
          !      (5) : for decapole
          !      (6) : for 12 pole
          ...
          !      (8) : for 16 pole
          !      (9) : radius
          !      (10) : x misalignment error
          !      (11) : y misalignment error
          !      (12) : rotation error x
          !      (13) : rotation error y
          !      (14) : rotation error z
        end type Multipole
...
!----------------------------------------------------------------
        ...
        double precision :: zedge,qmcc,brho,bgrad,zz,bstr2,bstr3

        zedge = this%Param(1)
        if(this%Param(3).gt.1.0e-5) then
          zz = t - zedge
          call getfldfrg_Multipole(zz,this,bgrad,bstr2,bstr3)
        else
          bgrad = this%Param(2)
        endif
!!QZ20120214    I'm not sure what the "IF-ELSE-ENDIF" does. To be checked if it is correct at all.
...
!------------------------------------------------------------------------
        ...
        double precision:: zz,bstr1,bstr2,bstr3,bstr4,bstr5,bstr6,bstr7,zedge
        ...
        extfld(4)=bstr1*pos(2)&
                 +bstr2*2*pos(2)*pos(1)&
                 +bstr3*(3*pos(1)**2*pos(2)-pos(2)**3)&
                 +bstr4*(4*pos(1)**3*pos(2)-4*pos(1)*pos(2)**3)&
                 +bstr5*(5*pos(1)**4*pos(2)-10*pos(1)**2*pos(2)**3+pos(2)**5)&
                 +bstr6*(6*pos(1)**5*pos(2)-20*pos(1)**3*pos(2)**3+6*pos(1)*pos(2)**5)&
                 +bstr7*(7*pos(1)**6*pos(2)-35*pos(1)**4*pos(2)**3+21*pos(1)**2*pos(2)**5-pos(2)**7)

        extfld(5)=bstr1*pos(1)&
                 +bstr2*(pos(1)**2-pos(2)**2)&
                 +bstr3*(pos(1)**3-3*pos(1)*pos(2)**2)&
                 +bstr4*(pos(1)**4-6*pos(1)**2*pos(2)**2+pos(2)**4)&
                 +bstr5*(pos(1)**5-10*pos(1)**3*pos(2)**2+5*pos(1)*pos(2)**4)&
                 +bstr6*(pos(1)**6-15*pos(1)**4*pos(2)**2+15*pos(1)**2*pos(2)**4-pos(2)**6)&
                 +bstr7*(pos(1)**7-21*pos(1)**5*pos(2)**2+35*pos(1)**3*pos(2)**4-7*pos(1)*pos(2)**6)
        ...
        subroutine getfldfrg_Multipole(zz,this,bstr1,bstr2,bstr3)
        implicit none
        include 'mpif.h'
        type (Multipole), intent(in) :: this
        double precision, intent(in) :: zz
        double precision, intent(out) :: bstr1,bstr2,bstr3
...
1       if(khi-klo.gt.1) then
        ...
        bstr1 =edat(klo)+slope*(zz-zdat(klo))
        slope=(epdat(khi)-epdat(klo))/hstep
        bstr2 =epdat(klo)+slope*(zz-zdat(klo))
        slope=(eppdat(khi)-eppdat(klo))/hstep
        bstr3 =eppdat(klo)+slope*(zz-zdat(klo))
