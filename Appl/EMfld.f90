...
!----------------------------------------------------------------
      module EMfldclass
        use PhysConstclass
        use Dataclass
        integer, private, parameter :: Nparam = 31
        type EMfld
          !Itype = 110
          ...
          !      (11) : rotation error y
          !      (12) : rotation error z
          !      (13) : flag for 3D discrete data(1),analytical+discret(2),analtyical(other)
          !      (14) : flag for Cartisian(2), Cylindrical coordintate(1)
          !      (15) : flag for synchronous phase input
          !      (16) : scale error
          !      (17) : theta0 error
          !      (18) : scale2
          !      (19) : RF frequency2
          !      (20) : theta2 (relative phase)
          !      (21) : scale3
          !      (22) : RF frequency3
          !      (23) : theta3 (relative phase)
          !      (24) : scale2 error
          !      (25) : theta2 error
          !      (26) : scale3 error
          !      (27) : theta3 error
          !      (28) : stored theta0 offset
          !      (29) : stored reference phase (internal parameter)
          !      (30) : stored reference gamma (internal parameter)
          !      (31) : stored reference qmcc (internal parameter)
      ...
      contains
        ...
        double precision :: uprimf,qpwf,len,zmid,fsync,erscl1,erphi1
        integer :: mpstp
        ...
        if (zedge.lt.0.0) then
            zedge = 0.0
            erscl1 = 0.0
            erphi1 = 0.0
        else
            erscl1 = this%Param(16)
            erphi1 = this%Param(17)
        endif
        
        escale = this%Param(2) + erscl1
        theta0 = (this%Param(4)+this%Param(28)+erphi1)*Pi/180.0
...
!        call getaxfldE_EMfld(t-zedge,escale,ein,e1in,e2in)
        call getaxfldEfc_EMfld(t-zmid,len,escale,ein,e1in,e2in)
        ...
        call rk6i_EMfld(h,mpstp,t0,y,18,zmid,len,escale,ww,theta0,Bchg,Bmass)
        ...
        sqf3=sqrt(uf)**3
        tfin = t + tau
!        call getaxfldE_EMfld(tfin-zedge,escale,ef,e1f,e2f)
        call getaxfldEfc_EMfld(tfin-zmid,len,escale,ef,e1f,e2f)
        ...
        subroutine rk6i_EMfld(h,ns,t,y,nvar,zmid,len,escale,ww,theta0,Bchg,Bmass)
        implicit none
        include 'mpif.h'
        ...
        tint=t
        do i=1,ns
          call intfunc1_EMfld(t,y,f,zmid,len,escale,ww,theta0,Bchg,Bmass)
          a=h*f
          yt=y+a/9.d0
          tt=t+h/9.d0
          call intfunc1_EMfld(tt,yt,f,zmid,len,escale,ww,theta0,Bchg,Bmass)
          b=h*f
          yt=y + (a + 3.d0*b)/24.d0
          tt=t+h/6.d0
          call intfunc1_EMfld(tt,yt,f,zmid,len,escale,ww,theta0,Bchg,Bmass)
        ...
        subroutine intfunc1_EMfld(t,y,f,zmid,len,escale,ww,theta0,Bchg,Bmass)
        implicit none
        include 'mpif.h'
        double precision, intent(in) :: t,zmid,len,escale,ww,theta0,Bchg,Bmass
        ...
        !call getaxfldE_EMfld(t,escale,ez1,ezp1,ezpp1)
        call getaxfldEfc_EMfld(t-zmid,len,escale,ez1,ezp1,ezpp1)
        qmcc = Bchg/Bmass
        ...
        subroutine getaxfldE_EMfld(zz,escale,ez1,ezp1,ezpp1)
        implicit none
        include 'mpif.h'
...
! Users should supply the field function as they want.
        subroutine  getaxfldEfc_EMfld(zz,len,escale,ez1,ezp1,ezpp1)
        implicit none
        include 'mpif.h'
...
! This is based on the assumption of azmuthal symmetry and TM mode.
        ...
        tmpbx = -pos(1)*ezp1/2+ezppp*pos(1)*r2/16
        tmpby = -pos(2)*ezp1/2+ezppp*pos(2)*r2/16
        tmpbz = ez1 - ezpp1*r2/4
...
!        write(11,102)pos(3),zz,ez1,ezp1
!102     format(4(1x,e14.5))
!        call flush_(11)

        end subroutine getfld_EMfld

        subroutine  getfldold_EMfld(pos,extfld,this)
        implicit none
        include 'mpif.h'
        double precision, dimension(4), intent(in) :: pos
        type (EMfld), intent(in) :: this
...
! This is based on the assumption of azmuthal symmetry and TM mode.
        ...
        end subroutine getfldold_EMfld
...
! given "z".
        subroutine getfld6xyz_EMfld(zz,extfld6xyz)
        double precision, intent(in) :: zz
...
! Here, we have only Er, Ez, and Btheta for TM mode.
        ...
        subroutine getfld6xyz_EMfld3(zz,extfld6xyz,extfld6xyz2,extfld6xyz3)
        double precision, intent(in) :: zz
        double precision, dimension(6,NxIntvRfg+1,NyIntvRfg+1), intent(out) ::&
        ...
        if (iz.ge.1 .and. iz1.le.NzIntvRfg+1) then
            do j = 1, NyIntvRfg+1
                do i = 1, NxIntvRfg+1
                    extfld6xyz(1,i,j) = Exgrid(i,j,iz)*ef + Exgrid(i,j,iz1)*(1.0-ef)
                    extfld6xyz(2,i,j) = Eygrid(i,j,iz)*ef + Eygrid(i,j,iz1)*(1.0-ef)
                    ...
                    if (Nmultigrid.eq.2 .or. Nmultigrid.eq.3) then
                        extfld6xyz2(1,i,j) = Exgrid2(i,j,iz)*ef + Exgrid2(i,j,iz1)*(1.0-ef)
                        extfld6xyz2(2,i,j) = Eygrid2(i,j,iz)*ef + Eygrid2(i,j,iz1)*(1.0-ef)
                        extfld6xyz2(3,i,j) = Ezgrid2(i,j,iz)*ef + Ezgrid2(i,j,iz1)*(1.0-ef)
                        extfld6xyz2(4,i,j) = Bxgrid2(i,j,iz)*ef + Bxgrid2(i,j,iz1)*(1.0-ef)
                        extfld6xyz2(5,i,j) = Bygrid2(i,j,iz)*ef + Bygrid2(i,j,iz1)*(1.0-ef)
                        extfld6xyz2(6,i,j) = Bzgrid2(i,j,iz)*ef + Bzgrid2(i,j,iz1)*(1.0-ef)
                    endif

                    if (Nmultigrid.eq.3) then
                        extfld6xyz3(1,i,j) = Exgrid3(i,j,iz)*ef + Exgrid3(i,j,iz1)*(1.0-ef)
                        extfld6xyz3(2,i,j) = Eygrid3(i,j,iz)*ef + Eygrid3(i,j,iz1)*(1.0-ef)
                        extfld6xyz3(3,i,j) = Ezgrid3(i,j,iz)*ef + Ezgrid3(i,j,iz1)*(1.0-ef)
                        extfld6xyz3(4,i,j) = Bxgrid3(i,j,iz)*ef + Bxgrid3(i,j,iz1)*(1.0-ef)
                        extfld6xyz3(5,i,j) = Bygrid3(i,j,iz)*ef + Bygrid3(i,j,iz1)*(1.0-ef)
                        extfld6xyz3(6,i,j) = Bzgrid3(i,j,iz)*ef + Bzgrid3(i,j,iz1)*(1.0-ef)
                    endif
                enddo
            enddo
        endif
        
        end subroutine getfld6xyz_EMfld3
        ...
        subroutine getbendref_EMfld(zadv, angle, zlfrg, zlbnd, refang, refxcen, refdz)
        implicit none
        double precision, intent(in) :: zadv, angle, zlfrg, zlbnd
        double precision, intent(out) :: refang, refxcen, refdz
        double precision :: crvtr, angprg, ztmp
        ! refang  : bended angle at zadv. [degree] 
        ! refxcen : horizontal reference point at zadv. [m]
        ! refdz   : z difference between bended coordinate and straight coordinate. [m]
        
        crvtr = 0.5*zlbnd/sin(0.5*angle)
        if (zadv .lt. zlfrg) then
            refang = -0.5*angle*180.0/Pi
            refxcen = zadv*tan(0.5*angle)
            refdz = zadv/cos(0.5*angle) - zadv
        else if (zadv .lt. zlfrg+zlbnd) then
            ztmp = zadv - zlfrg
            angprg = - atan((sin(0.5*angle)-ztmp/crvtr)/cos(0.5*angle))
            refang = angprg*180.0/Pi
            refxcen = zlfrg*tan(0.5*angle) + crvtr*(cos(angprg)-cos(0.5*angle))
            refdz = (zlfrg/cos(0.5*angle)-zlfrg) + (crvtr*(0.5*angle-asin(sin(0.5*angle)-ztmp/crvtr)) - ztmp)
        else
            refang = 0.5*angle*180.0/Pi
            refxcen = (zlbnd + 2.0*zlfrg -zadv)*tan(0.5*angle)
            ztmp = zadv - zlfrg - zlbnd
            refdz = (zlfrg/cos(0.5*angle)-zlfrg) + (crvtr*angle - zlbnd) + (ztmp/cos(0.5*angle) - ztmp)
        endif
        
        end subroutine getbendref_EMfld
