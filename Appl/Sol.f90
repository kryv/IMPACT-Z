...
!        anglez = this%Param(9)
        ...
        !linear map solenoid model (modified versions) 
        subroutine linearmod_Sol(innp, rays, tau, gam, al, gtf)
        implicit none
        integer, intent(in) :: innp
        double precision, dimension(9, innp), intent(inout) :: rays
        double precision, intent(in) :: tau, gam, al, gtf
        
        integer :: n
        double precision, dimension(9) :: tmpray, tmpray2
        double precision :: gamn, gambetz, ainvp, ak, akal, xtt, ytt, xbt ,ybt, beta0
        
        beta0 = sqrt(gam**2-1.0)/gam
        tmpray = 0.0
        do n = 1, innp
            tmpray(1) = Scxl*rays(1,n)*100.0 !in cm
            gamn = gam - rays(6,n)
            gambetz = sqrt(gamn**2 - 1 - rays(2,n)**2 - rays(4,n)**2)
            tmpray(2) = rays(2,n)/gambetz
            tmpray(3) = Scxl*rays(3,n)*100
            tmpray(4) = rays(4,n)/gambetz
  
            ainvp = rays(7,n)/gambetz
            ak = 0.5*Clight*100*1.0e-8*gtf*ainvp   
            akal = ak*al
            xtt = cos(akal)**2*tmpray(1)+1.0/ak*sin(akal)*cos(akal)*&
                  tmpray(2)+sin(akal)*cos(akal)*tmpray(3)+ &
                  1.0/ak*sin(akal)**2*tmpray(4)
            xbt = -ak*sin(akal)*cos(akal)*tmpray(1)+&
                  cos(akal)**2*tmpray(2)-ak*sin(akal)**2*tmpray(3)+&
                  sin(akal)*cos(akal)*tmpray(4) 
            ytt = -sin(akal)*cos(akal)*tmpray(1)-&
                  1.0/ak*sin(akal)**2*tmpray(2)+cos(akal)**2*tmpray(3)+&
                  1.0/ak*sin(akal)*cos(akal)*tmpray(4)
            ybt = ak*sin(akal)**2*tmpray(1)-sin(akal)*cos(akal)*tmpray(2)-&
                  ak*sin(akal)*cos(akal)*tmpray(3)+cos(akal)**2*tmpray(4)

            tmpray2(1) = xtt/100
            tmpray2(2) = xbt
            tmpray2(3) = ytt/100
            tmpray2(4) = ybt
            rays(1,n) = tmpray2(1)/Scxl
            rays(3,n) = tmpray2(3)/Scxl
            gambetz=sqrt((gamn**2-1)/(1.0+tmpray2(2)**2+tmpray2(4)**2))
            rays(2,n) = tmpray2(2)*gambetz
            rays(4,n) = tmpray2(4)*gambetz
            rays(5,n) = rays(5,n)+tau*((gam-rays(6,n))/(Scxl*gambetz)-1.0/(beta0*Scxl))

        enddo
        
        end subroutine linearmod_Sol
